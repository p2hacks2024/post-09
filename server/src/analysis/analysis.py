...
class BaseOutput(BaseModel):
    ...
    '''
    emotion_freq: dict
    emotion_to_situation: dict
    situation_to_emotion_freq: dict
...
class Analysis:
    ...
    def _get_base_output(self, period: str) -> BaseOutput:
        ...
        emotion_to_situation = defaultdict(list)
        situation_to_emotion_freq = defaultdict(lambda: defaultdict(int))

        # 各要素の集計
        for activity in self.activities:
            activity_timestamp = datetime.strptime(activity.timestamp, '%Y-%m-%dT%H:%M:%S')
            activity_period = getattr(activity_timestamp, period) if period != 'total' else None
        ...
        return BaseOutput(
            ...
            situation_to_emotion_freq=situation_to_emotion_freq
        )